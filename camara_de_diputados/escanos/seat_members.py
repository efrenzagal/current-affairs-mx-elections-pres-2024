"""Resolve LXVI Camara roll-call identities to constitutional seats.

Roll-call rows carry no geography: `dim_gaceta_deputy` is an id and a name. Turning
...
The Senado runs the same resolution over its own registers in
`camara_de_senadores/escanos/seat_members.py`. The two write disjoint
...
Usage::
    ...
    python -m camara_de_diputados.escanos.seat_members
"""
...
from camara_de_diputados.escanos.audited_overrides import (  # noqa: E402
    load_person_aliases,
    load_seat_overrides,
)
...
def load_current_roster(conn: sqlite3.Connection) -> tuple[dict[str, dict], dict]:
    ...
    if not snapshot:
        raise SystemExit(
            "No roster snapshot for DIP."
            " Run camara_de_diputados/composicion/ingest.py first."
        )
    meta = snapshot[0]
    roster = {
        row["seatId"]: row
...
def load_substitutes(conn: sqlite3.Connection) -> dict[str, str]:
    ...
    return {
        ...
        for row in rows(
            conn,
            """
            SELECT diputado_id AS seatId, ine_substitute_name AS name
            FROM dim_diputados
...
def load_chamber_vote_rows(conn: sqlite3.Connection) -> list[dict]:
    """Roll-call rows for this chamber, already in the Camara's vocabulary.

    The Senado has to translate its own PRO/CONTRA/ABSTENCIÓN/AUSENTE into these
    labels so a merged history reads the same either side of the building; the
    Camara records Favor/Contra/Abstención/Ausente directly.
    """
    return rows(conn, VOTE_ROWS_SQL)
