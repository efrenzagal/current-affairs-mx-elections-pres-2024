...
maps the titular—or the officially listed substitute who actually appears in
...
# Manually verified aliases where the official INE integration and the Gaceta
# refer to the same person but the conservative token matcher cannot prove it.
# Keep these explicit: loosening the global fuzzy-match threshold risks linking
# two different deputies with similar names.
AUDITED_GACETA_NAME_OVERRIDES = {
    "CESAR ALEJANDRO DOMINGUEZ DOMINGUEZ": "Domínguez Domínguez César A.",
    "TECUTLI JOSE GUADALUPE GOMEZ VILLALOBOS": "Gómez Villalobos Tecutli José G.",
    "FUENSANTA GUADALUPE GUERRERO ESQUIVEL": "Guerrero Esquivel Fuensanta G.",
    "FRANCISCO ARTURO FEDERICO AVILA ANAYA": "Avila Anaya Francisco Arturo F.",
    "ROCIO ADRIANA ABREU ARIÑANO": "Abreu Artiñano Rocío Adriana",
    "MAIELLA MARTHA GABRIELA GOMEZ MALDONADO": "Gómez Maldonado Maiella Martha G.",
    "OLGA DEL CARMEN SANCHEZ CORDERO DAVILA": "Sánchez Cordero Dávila Olga",
}
...
def resolve_gaceta_identity(
    candidate_name: str,
    substitute_name: str,
    roster_names: list[str],
) -> tuple[str | None, str | None, str, float | None]:
    """Resolve an official seat to the person appearing in Gaceta.
    ...
    roll-call history. Audited aliases cover source typos and Gaceta initials
    without weakening the conservative matcher for everybody else.
    """
    ...
    for role, identity_name in identities:
        ...
        audited_name = AUDITED_GACETA_NAME_OVERRIDES.get(identity_name.upper())
        if audited_name is not None:
            if audited_name not in roster_set:
                raise ValueError(
                    f"Audited Gaceta identity not found in legislature {LEGISLATURE} "
...
def build_dim_diputados(
    ...
    for _, source in official.iterrows():
        ...
        if not candidate_name and not substitute_name:
            ...
            matched_name, matched_role, match_method, match_score = resolve_gaceta_identity(
                candidate_name,
                substitute_name,
                roster_names,
            )
...
def validate_dim_diputados(
    ...
    unmatched = conn.execute(
        f"SELECT COUNT(*) FROM {table_name} WHERE gaceta_deputy_id IS NULL"
    ).fetchone()[0]
    if unmatched:
        raise ValueError(f"dim_diputados contains {unmatched} seats without Gaceta mappings")
